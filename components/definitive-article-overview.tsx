...
import type { BlogPost, Chapter } from "@/lib/types";
...
export function DefinitiveArticleOverview({ post }: DefinitiveArticleOverviewProps) {
  ...
  const totalArticles = chapters.reduce((acc, chapter) => acc + (chapter.posts?.length || 0), 0);
  const totalReadTime = chapters.reduce((acc, chapter) => {
    return acc + (chapter.posts?.reduce((postAcc, post) => {
  ...
  if (chapters.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-xl font-semibold text-slate-600 dark:text-slate-400">No chapters available</h3>
        <p className="text-slate-500 dark:text-slate-500">This definitive guide is still being prepared.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <Link href={`/author/${post.author.slug}`} className="font-semibold text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm md:text-base">
                    {post.author.name}
                  </Link>
      ...
      <div className="lg:hidden sticky top-0 z-40 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3">
        <button
          ...
          return (
            ...
            >
              ...
              <div className="bg-white dark:bg-slate-800 rounded-xl md:rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-lg mb-6 md:mb-8">
                <div className={`bg-gradient-to-r ${chapterColors.bg} p-4 md:p-6`}>
                ...
                <div className="p-4 md:p-6 bg-slate-50 dark:bg-slate-700/30">
                  <div className="flex items-center justify-between text-xs md:text-sm">
                    ...
                    <div className="hidden sm:flex items-center space-x-2">
                      ...
                      <span className="text-slate-600 dark:text-slate-400 text-xs">Ready to read</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {chapterPosts.length > 0 && (
                <div className="space-y-4 md:space-y-6">
                  {chapterPosts.map((chapterPost, postIndex) => (
                    <article key={chapterPost.id} className="bg-white dark:bg-slate-800 rounded-lg md:rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-lg transition-shadow">
                      {/* Post Header */}
                      <div className="p-4 md:p-6 border-b border-slate-200 dark:border-slate-700">
                        <div className="flex items-start space-x-3 md:space-x-4">
                          <div className={`w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br ${chapterColors.bg} rounded-md md:rounded-lg flex items-center justify-center text-white font-semibold text-xs md:text-sm flex-shrink-0`}>
                            {postIndex + 1}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center space-x-3 md:space-x-4 text-xs md:text-sm text-slate-500 dark:text-slate-500">
                              ...
                              <div className="hidden sm:flex items-center space-x-1">
                                ...
                                <span>{new Date(chapterPost.publishedAt).toLocaleDateString()}</span>
                              </div>
                            </div>
                          </div>
                          {chapterPost.image && (
                        ...
                        {chapterPost.content ? (
                          ...
                          <div className="text-center py-6 md:py-8 text-slate-500 dark:text-slate-400">
                            <div className="max-w-sm mx-auto">
                              ...
                              <p className="font-medium mb-2 text-sm md:text-base">Content not available</p>
                              <p className="text-xs md:text-sm mb-4">This chapter content is still being prepared.</p>
                              <Link 
